        ...
        <div className="p-10 overflow-auto rounded-xl" id="music-repositories">
            {data !== undefined ? (
                data.map((score: Score, index: number) => (
                    <div className="flex my-5" id={score.title} key={index}>
                        <div className="m-0 p-0 mr-4">
                            <a className="mr-10" href={score.href} target="_blank" rel="noopener noreferrer">
                                <img className="max-h-32" src={score.thumbnail} alt={score.title}></img>
                            </a>
                        </div>
                        <div id={"info-" + (index+1)}>
                            <a href={score.href} target="_blank" rel="noopener noreferrer">
                                <p className="hyperlink text-xl" id="title" key="title">{(index+1) + ". " + score.title}</p>
                            </a>
